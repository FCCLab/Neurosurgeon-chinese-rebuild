...
"""
    Cloud device API used to receive intermediate data and execute the remaining DNN parts on the cloud,
    saving the results in an Excel spreadsheet.
    Server start command: python cloud_api.py -i 127.0.0.1 -p 9999 -d cpu
    "-i", "--ip"            Server IP address
    "-p", "--port"          Port opened by the server
...
if __name__ == '__main__':
    ...
    # Process options as tuples (opt, arg)
    ...
    if device == "cuda" and torch.cuda.is_available() == False:
        raise RuntimeError("CUDA cannot be used on this machine.")
