...
def start_monitor_client(ip, bandwidth_value):
    """
    Start: Bandwidth monitoring client
    :param ip: Corresponding client IP address
    :param bandwidth_value: Bandwidth data to be updated using a shared variable to enable inter-process communication
    :return:
    """
...
def scheduler_for_bandwidth_monitor_edge(ip, interval, bandwidth_value):
    """
    :param ip: Corresponding client IP address
    :param interval: Timer interval for scheduling
    :param bandwidth_value: Bandwidth data to be updated using a shared variable to enable inter-process communication
    :return:
    """
    # Create scheduler
    scheduler = BackgroundScheduler(timezone='MST')
    # Create a bandwidth monitoring process every 'interval' seconds to fetch the latest bandwidth data
    scheduler.add_job(start_monitor_client, 'interval', seconds=interval, args=[ip, bandwidth_value])
    scheduler.start()


"""
    Edge device API used to start the edge device, compute the first half of the model, 
    and send intermediate data to the cloud device.
    Client start command: python tasks_edge_api.py -i 127.0.0.1 -p 9999 -d cpu
    "-i", "--ip"           Server IP address
    "-p", "--port"         Server open port
    "-d", "--device"       Whether to enable GPU computation on the client (cpu or cuda)
"""
if __name__ == '__main__':
    ...
    # Process the options, which are stored as tuples (opt, arg)
    ...
    # Randomly create a task queue with a total of 40 DNN inference tasks
    ...
    # Use a shared variable, pre-set interval for bandwidth monitoring is 1 second
    bandwidth_value = multiprocessing.Value('d', 0.0)

    # Get the bandwidth once before using the scheduler to continuously monitor bandwidth
    monitor_cli = MonitorClient(ip=ip, bandwidth_value=bandwidth_value)
    monitor_cli.start()
    monitor_cli.join()
    ...
    for task_name in tasks_list:
        ...
        # Deployment phase - Choose the optimized partition point
        ...
        # Start inference using collaborative edge-cloud processing: Establish one connection
